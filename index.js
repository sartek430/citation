...
bouton.addEventListener("click", () => {
  do {
    function genererNombreEntier() {
      nombreAlatoire = Math.floor(Math.random() * Math.floor(citations.length));
      return nombreAlatoire;
    }
  } while (genererNombreEntier() == dernier);

  citation.innerHTML = citations[nombreAlatoire][0];
  auteur.innerHTML = citations[nombreAlatoire][1];
  dernier = genererNombreEntier();
});
